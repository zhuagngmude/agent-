...
`AGENT_SWARM_ENABLE_MODEL_CONNECTIVITY_TEST` is currently a visible request flag only. Even when that environment variable is `true`, MVP-0.5 must keep `manualConnectivityTestActive=false` and `realProviderRequestsAllowed=false`.
