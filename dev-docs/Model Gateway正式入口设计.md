...
- 已有 redaction / response limiter helper 和安全 `model_calls` 记录草稿，当前不写业务记录、不写 Mock 运行态。
- 阶段 23 已在 Tauri/Rust 新架构中建立 `model_calls` SQLite 审计表；当前仍不落盘调用记录、不写 runtime event。
...
- 本文只补正式入口设计和 provider 配置 / Key 存储方案；当前实现仍不发 provider 请求，不写 `model_calls` 业务记录。
