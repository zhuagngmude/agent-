...
## 当前先行 MVP：一个执行器、一个模型、一条真链路

在完整 P0 表结构落地之前，先做一个最小可验证链路：

```text
1 个总控 Agent
+ 1 个默认执行器：model_gateway_default
+ 1 个默认模型：系统设置中的模型 ID
+ 1 条真实调用链：主控台 -> 自动任务 -> Runner -> workspace/generated
```
...
1. 在系统设置里配置 Base URL、API Key、模型 ID。
2. 点击模型连接测试，确认连接可用。
3. 回到主控台输入一个简单网页目标，执行全自动流程。
4. 在任务页看到任务状态变化。
5. 在 `workspace/generated` 看到生成的项目文件。
