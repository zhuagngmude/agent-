...
- `scripts/sqlite/` 存放 Python 桥接脚本，避免在 PowerShell 或 Node.js 中维护大段内联 Python。
...
7. 已新增 SQLite 状态流转写入层，支持任务、审批、Agent 配置申请/应用/取消写入 SQLite。
8. 已将 SQLite 初始化、seed 和写入 Python 逻辑抽到 `scripts/sqlite/`。
9. 已新增 `scripts/verify-sqlite-flows.ps1`，用于独立验证 SQLite 模式。
...
`GET /api/projects/:projectId/dashboard`、`/agents`、`/tasks`、`/approvals`、`/workflows`、`/agent-config-applications`、`/runner/status`、`/runner/jobs`、`/git/checkpoints`、`/knowledge/updates` 已支持环境变量开关的 SQLite 读取，默认仍使用 Mock 数据。

任务 action、审批 action、Agent 配置申请/应用/取消已在 SQLite 模式下写入 SQLite；Runner job 仍只生成只读队列，不执行任何本地命令。
