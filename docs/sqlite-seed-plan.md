...
- `services/api/db/` 后续放数据库连接、查询和 row mapper。
...
后续实现 seed 时建议先做一个可提交的 JSON 快照，而不是让 SQL 脚本直接解析 JavaScript：
...
1. 新增 `.gitignore` 规则，忽略 `data/local/` 和 SQLite wal/shm 文件。
2. 新增 `data/migrations/001_initial_sqlite.sql`，只创建第一版表和索引。
3. 新增 `scripts/init-sqlite.ps1`，创建 `data/local/agent-swarm.sqlite` 并执行 migration。
4. 新增 `data/seed/project_agent_swarm.seed.json`，保存从 Mock 数据导出的初始快照。
5. 新增 `scripts/seed-sqlite.ps1`，清空第一版表并按依赖顺序写入 seed。
6. 新增只读数据库查询层，先让 Dashboard 从 SQLite 读取。
7. 再迁移任务、审批、Agent 配置应用/取消等状态流转。
