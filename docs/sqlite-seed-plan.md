...
6. 已新增只读数据库查询层，可通过 `AGENT_SWARM_DASHBOARD_SOURCE=sqlite` 让 Dashboard 从 SQLite 读取。
7. 下一步迁移 `GET /api/projects/:projectId/agents` 等只读接口。
...
`GET /api/projects/:projectId/dashboard` 已支持环境变量开关的 SQLite 只读读取，默认仍使用 Mock 数据。
