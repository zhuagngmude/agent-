...
6. 已新增只读数据库查询层，可通过 `AGENT_SWARM_DASHBOARD_SOURCE=sqlite` 让 Dashboard、Agents、Tasks、Approvals 和 Workflows 从 SQLite 读取。
7. 下一步可继续迁移 Runner jobs、Agent config applications、Git checkpoints、Knowledge updates 等只读接口。
8. 再迁移任务、审批、Agent 配置应用/取消等状态流转。
...
`GET /api/projects/:projectId/dashboard`、`/agents`、`/tasks`、`/approvals`、`/workflows` 已支持环境变量开关的 SQLite 只读读取，默认仍使用 Mock 数据。
