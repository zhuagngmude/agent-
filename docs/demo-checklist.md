...
- `verify-agent-config-rollback-request.ps1` covers the helper-only Agent config rollback request contract: a valid request may draft a future approval/application/version, but must keep `ok=false`, `canCreateApproval=false`, `feature_disabled`, and all side effects false.
