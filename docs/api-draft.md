...
## MVP-0.3 Project Plan Approval API

Detailed spec: `docs/mvp-0.3-project-plan-flow-spec.md`.

MVP-0.3 adds a local project idea -> project plan approval -> Agent task split -> read-only Runner request queue flow. It is implemented in Mock and SQLite modes, but it still does not call real models, execute Runner, write project files, make network requests, modify Git, sync cloud state, or enable a full permission system.

### POST /api/projects/:projectId/project-plan-requests

Creates a pending `project_plan` approval from a user project idea and constraints.
...
- `idea` is required.
- The draft is generated by `local_deterministic_template`, not by a real model.
- The approval uses `targetService=project_plan`.
- The approval must keep `runnerJobId=""`.
- Draft creation may persist the approval, but must not create tasks or Runner request records before user approval.
- All real side effects must stay false: no project file write, Git change, Runner execution, model call, raw-secret read, network request, or Agent trigger.

### Project Plan Approval

Approving a pending `project_plan` approval creates five queued tasks and five read-only Runner request queue records:
...
```json
{
  ...
  "runnerJobId": "",
  "createdTaskIds": [
    "task_local_customer_lead_tracker_frontend"
  ],
  "createdRunnerJobIds": [
    "runner_job_task_local_customer_lead_tracker_frontend"
  ],
  "sideEffects": {
...
`createsRunnerJobs=true` here means local read-only Runner request queue records were created. It does not mean Runner execution is enabled.
