...
>
> 2026-06-19 更新：P0 AI 员工到 Runner 的 6 个 checkpoint 已完成。当前优先看 Tauri commands 和 Rust services；旧 Node.js HTTP route、禁用态 Model Gateway 草案和只读 Runner request 段落保留为历史参考，不作为当前主线待办。
...
### Tauri/Rust project_plan commands

当前新架构不继续扩展旧 Node.js HTTP route，而是通过 Tauri invoke 暴露明确 command：
...
### Tauri/Rust agent_config commands（P0 已收口）
...
- `list_agent_boundary_checks` 只读，用来展示后端越界判断记录。
- `recommend_project_agents` 第一版使用确定性规则，不调用模型；输入项目类型、技术栈、风险、阶段，返回推荐核心员工和专家。
- `assign_project_agents_to_task` 将任务分配到真实 `project_agents.id`，后端会校验任务与项目 Agent 都属于当前项目。
- `check_agent_boundary` 在 Runner preflight 前校验 agent_id、task_type、module_scope、target_path、forbidden_actions，结果只能是 `allowed`/`denied`/`needs_approval`，每次检查写入 `agent_boundary_checks`；`denied` 会阻断 preflight，旧 `agents` 表 ID 仍走兼容路径。
- `get_task_agent_info` 按任务 ID 查询绑定的项目 Agent 信息（名称、角色、执行器、模型、模块范围），已用于任务页展示 Agent 闭环；运行输出页后续继续增强。
