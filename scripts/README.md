...
Future real Model Gateway manual connectivity testing must remain behind an explicit disabled-by-default gate. No script in this directory should require real provider API keys, install provider SDKs, call OpenAI/Anthropic/Gemini, store provider responses, or turn disabled stub checks into real connectivity checks.
