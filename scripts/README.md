...
- API health, runtime state, Runner safety flags, `GET /api/model-gateway/status`, and `POST /api/model-gateway/dry-run`.
...
Future Model Gateway manual connectivity testing is specification-only. No script in this directory should require real provider API keys, install provider SDKs, call OpenAI/Anthropic/Gemini, store provider responses, or turn dry-run checks into real connectivity checks until the API spec and acceptance rules are implemented behind an explicit disabled-by-default gate.
