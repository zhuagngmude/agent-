...
`verify-mock-flows.ps1` 会在隔离端口 `8789` 启动 Mock API，验证 Mock API 的关键状态流转，并在结束后重置本地 runtime state。它还会检查：MVP-0.3 project plan request 在审批前只创建/更新 `project_plan` 审批，审批后才生成五个 Agent 分工任务和五条 `runner_request_readonly` 队列记录；非法 Agent permission change request 在创建审批前被拒绝；已批准的 Agent config 变更只创建 `pending_apply` application 记录，不创建 Runner job，也不进行真实 Agent 配置写入；Agent config dry-run 保持 feature-disabled，且所有 sideEffects 为 false。

`verify-sqlite-flows.ps1` 会在隔离端口 `8788` 启动 SQLite 模式 API，验证 Dashboard、任务、审批、Runner job、Agent 配置应用/取消和 reset 状态重建。它还会检查：MVP-0.3 project plan request 在 SQLite 审批前不写任务或 Runner request queue，审批后写入五个 Agent 分工任务、五条 `runner_request_readonly` 队列记录和对应 runtime events；非法 Agent permission change request 在 SQLite 写入前被拒绝；已批准的 Agent config 变更只创建 `pending_apply` application 记录，不创建 Runner job，也不进行真实 Agent 配置写入；Agent config dry-run 保持 feature-disabled，且所有 sideEffects 为 false。
...
`verify-project-plan-flow.ps1` is the MVP-0.3 helper contract check for the project idea -> plan approval -> Agent task split -> read-only Runner request queue flow.

- It imports `services/api/project-plan.js` directly and does not start the API server.
- It verifies deterministic local plan draft creation, `targetService=project_plan` approval shape, frontend/backend/qa/docs/reviewer task assignment, read-only Runner request shape, idempotent instantiation, invalid input rejection, and no real side effects.
- It must not write SQLite, write runtime state, create project files, modify Git, execute Runner, call real models, make network requests, trigger Agents, or read raw secrets.
